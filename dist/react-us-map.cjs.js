...
var objectStrategyMaker     = object$1;
...
var throttleDebounce = cjs;
var createResizeDetector = elementResizeDetector;
var isShallowEqual = shallowequal;
...
var invariant__default = /*#__PURE__*/_interopDefaultLegacy$1(invariant);
var createResizeDetector__default = /*#__PURE__*/_interopDefaultLegacy$1(createResizeDetector);
var isShallowEqual__default = /*#__PURE__*/_interopDefaultLegacy$1(isShallowEqual);
...
var topoStates = /*#__PURE__*/Object.freeze({
  __proto__: null,
  type: type,
  objects: objects,
  arcs: arcs,
...
var adjustments = [
	{
		...
		label_adj_x: 0,
		label_adj_y: 0,
		mobile_label_adj_x: 1,
		mobile_label_adj_y: 0
	},
	{
		state: "Alaska",
		label_adj_x: 0,
		label_adj_y: 0,
		mobile_label_adj_x: 2,
		mobile_label_adj_y: 0
	},
	{
		state: "Arizona",
	...
	},
	{
		...
		label_adj_x: -10,
		label_adj_y: 0,
		mobile_label_adj_x: -3,
		mobile_label_adj_y: 2
	},
	{
		state: "Colorado",
		label_adj_x: 0,
	...
	},
	{
		...
		label_adj_x: 12,
		label_adj_y: 0,
		mobile_label_adj_x: 7,
		mobile_label_adj_y: 8
	},
	{
		state: "Georgia",
		label_adj_x: 0,
		label_adj_y: 0,
		mobile_label_adj_x: 0,
		mobile_label_adj_y: 3
	},
	{
		state: "Hawaii",
		label_adj_x: -20,
		label_adj_y: 10,
		mobile_label_adj_x: -12,
		mobile_label_adj_y: 6
	},
	{
		state: "Idaho",
		label_adj_x: -3,
	...
	},
	{
		...
		label_adj_x: 12,
		label_adj_y: 30,
		mobile_label_adj_x: 5,
		mobile_label_adj_y: 13
	},
	{
		state: "Minnesota",
		label_adj_x: 0,
...
const stateGeoData = feature(topoStates, objects['states']);
const mapLabelAdjustmentsByState = adjustments.reduce((p, v) => {
  p[v.state] = v;
  return p;
}, {});
...
const USMap = props => {
  ...
  React$1.useEffect(() => {
    ...
    const mouseover = (e, d) => {
      ...
      svg.selectAll(`.d3-state-${slugify(d)}`).style('fill', d => fillHover(d.properties.name)).style('stroke', d => strokeHover(d.properties.name)).style('stroke-width', d => strokeWidthHover(d.properties.name)).raise();
      svg.selectAll(`.d3-state-offshore-${slugify(d)}`).style('fill', d => fillHover(d.state)).style('stroke', d => strokeHover(d.state)).style('stroke-width', d => strokeWidthHover(d.state));
    };

    const mousemove = (e, d) => {
      tooltip.style('left', e.pageX > width * 0.75 ? e.pageX - tooltipWidth + 'px' : e.pageX + 10 + 'px').style('top', e.pageY + 10 + 'px');
    };

    const mouseout = (e, d) => {
      tooltip.style('opacity', 0).style('visibility', 'hidden');
      svg.selectAll(`.d3-state-${slugify(d)}`).style('fill', d => fill(d.properties.name)).style('stroke', d => stroke(d.properties.name)).style('stroke-width', d => strokeWidth(d.properties.name)); // TODO .lower() but only sometimes, depending on sort
    ...
    const statesLabeledOffshore = offshore.map(d => d.state);

    if (textBackgroundFill !== nop) {
      g.append('g').attr('class', 'state-labels-backgrounds').selectAll('.state-label-background').data(stateGeoData.features.filter(d => {
        return !statesLabeledOffshore.includes(d.properties.name) && textFilter(d.properties.name);
      })).enter().append('rect').attr('class', 'state-label-background').attr('x', d => {
      ...
      }).attr('width', d => textBackgroundWidth(d.properties.name)).attr('height', d => textBackgroundHeight(d.properties.name)).style('fill', d => textBackgroundFill(d.properties.name));
    }

    const stateLabels = g.append('g').attr('class', 'state-labels').selectAll('.state-label').data(stateGeoData.features.filter(d => {
      return !statesLabeledOffshore.includes(d.properties.name) && textFilter(d.properties.name);
    })).enter().append('text').attr('class', 'state-label').attr('x', d => {
      const adjustment = mapLabelAdjustmentsByState[d.properties.name][isSkinny ? 'mobile_label_adj_x' : 'label_adj_x'];
      return path.centroid(d)[0] + adjustment;
    }).attr('y', d => {
      const adjustment = mapLabelAdjustmentsByState[d.properties.name][isSkinny ? 'mobile_label_adj_y' : 'label_adj_y'];
      return path.centroid(d)[1] + adjustment;
    }).style('text-anchor', 'middle').style('font-size', fontSize).style('font-family', d => textFontFamily(d.properties.name)).style('font-weight', d => textFontWeight(d.properties.name)).style('paint-order', 'stroke').style('pointer-events', 'none').text(d => fullToPostal(d.properties.name)); // Don't apply fill or strokes to Hawaii since its label is not over the state
...
exports.USMap = WrappedUSMap;
exports.USMapUnwrapped = USMap;
